...
import static com.HiveStay.util.AppUtils.getCurrentUser;
...
public class BookingServiceImpl implements BookingService{
    ...
    public void capturePayment(Event event) {
        if ("checkout.session.completed".equals(event.getType())) {
            // getObject() silently returns null on Stripe API version mismatch.
            // Parse only the session ID we need directly from the raw JSON using Jackson.
            String sessionId;
            try {
                ...
                log.error("Failed to parse session ID from Stripe webhook event", e);
                return;
            }
